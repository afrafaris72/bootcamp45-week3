...
func detailProject(c echo.Context) error {
	...
	err := connection.Conn.QueryRow(context.Background(), "SELECT tb_project.id, tb_project.title, tb_project.content, tb_project.tech, tb_project.start_date, tb_project.end_date, tb_project.duration, tb_project.image, tb_user.name FROM tb_project inner join tb_user ON tb_project.author_id = tb_user.id where tb_project.id = $1", id).Scan(&ProjectDetail.Id, &ProjectDetail.Title, &ProjectDetail.Content, &ProjectDetail.Tech, &ProjectDetail.StartDate, &ProjectDetail.EndDate, &ProjectDetail.Duration, &ProjectDetail.Image, &ProjectDetail.Author)
